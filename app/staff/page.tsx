...
import { createTicket, getTicketStatus } from '@/app/actions/tickets'
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="bg-white rounded-2xl shadow p-6 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="rounded-xl border p-4 text-center text-gray-500">Score 1 (Pending Phase 3)</div>
            <div className="rounded-xl border p-4 text-center text-gray-500">Score 2 (Pending Phase 3)</div>
            <div className="rounded-xl border p-4 text-center text-gray-500">Score 3 (Pending Phase 3)</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            ...
            {raiseBanner && (
              <div className={`mt-4 rounded-lg px-4 py-3 text-sm font-medium ${
                raiseBanner.type === 'success' ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'
              }`}>
                {raiseBanner.message}
              </div>
            )}
          ...
          <div className="bg-white rounded-2xl shadow p-6 border-t-4 border-gray-800">
            ...
            <form ref={statusFormRef} action={handleStatusSubmit} className="space-y-4">
              <div>
                <label htmlFor="ticket_code" className="block text-sm font-medium text-gray-700 mb-1">
                  6-Digit Ticket Code
                </label>
                <div className="flex gap-2">
                  <input
                    ...
                    className="w-full rounded-lg border border-gray-300 px-4 py-2 outline-none focus:ring-2 focus:ring-gray-500"
                  />
                  <button
                    type="submit"
            ...
            {ticketDetails && (
              <div className="mt-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
                ...
                {/* Replies Timeline */}
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase mb-2">Admin Replies</p>
                  {ticketDetails.ticket_replies?.length === 0 ? (
                    <p className="text-sm text-gray-400 italic">No replies yet. Please check back later.</p>
                  ) : (
                    <div className="space-y-3">
                      {ticketDetails.ticket_replies.map((reply: any, idx: number) => (
                        <div key={idx} className="bg-blue-50 border-l-4 border-blue-500 p-3 rounded-r-lg">
                          ...
                          <p className="text-xs text-blue-400 mt-1">{new Date(reply.created_at).toLocaleString()}</p>
                        </div>
                      ))}
                    </div>
                  )}
