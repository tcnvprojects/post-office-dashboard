...
function DashboardContent() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (structureResult && structureResult.data) {
  ...
  async function handleEscalation() {
    if (ticketDetails && ticketDetails.id) {
      ...
      if (refreshed && refreshed.data) {
        setTicketDetails(refreshed.data)
      }
    }
  }

  // --- CALCULATION LOGIC ---
  const calculateVerticalScore = (vertical: any) => {
    if (!officeData || !officeData.metrics) {
      return 0
    }
    
    const parameterIds = vertical.parameters?.map((p: any) => p.id) || []
    const metricsForVertical = officeData.metrics.filter((m: any) => parameterIds.includes(m.parameter_id))
    
    if (metricsForVertical.length === 0) {
      return 0
    }
    
    const totalValue = metricsForVertical.reduce((sum: number, metric: any) => {
      return sum + (Number(metric.actual_value) || 0)
    }, 0)
    
    return Math.min(100, Math.max(0, Math.round(totalValue / metricsForVertical.length)))
  }
          ...
          <div className="space-y-4">
            {!selectedVertical ? (
              <>
                ...
                {structure.map((vertical: any) => {
                  const verticalScore = calculateVerticalScore(vertical)
                  return (
                    <button 
                      key={vertical.id} 
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
            ...
            <form ref={formRef} action={handleRaise} className="space-y-4">
              <textarea 
                name="description" 
                required 
                className="w-full border p-4 rounded-xl bg-gray-50 text-sm" 
            ...
            <div className="border-t pt-6">
              <form action={checkTicketStatus} className="flex gap-2">
                <input 
                  name="code" 
                  required 
                  className="border bg-gray-50 p-4 rounded-xl text-sm w-full" 
                ...
                <div className="mt-4 bg-gray-50 p-6 rounded-2xl text-sm space-y-2 border">
                  ...
                  {ticketDetails.admin_reply && (
                    <div className="mt-2 bg-white p-3 rounded-lg border border-gray-200">
                      <p className="text-[10px] font-bold text-gray-400 uppercase">Admin Reply:</p>
                      <p>{ticketDetails.admin_reply}</p>
                    </div>
                  )}
                    ...
                    <div className="mt-2">
                      {!isEscalating ? (
                        <button 
                          ...
                          className="text-red-600 underline font-bold text-xs"
                        >
                          Escalate to re-open?
                        </button>
                      ) : (
